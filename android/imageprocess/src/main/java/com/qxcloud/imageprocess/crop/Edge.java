...
public enum Edge {
    ...
    // becoming too small.
    public static final int MIN_CROP_LENGTH_PX = 80;//****cfh****update控制裁剪框最小高度
    public static int MIN_CROP_LENGTH_PX_W =80;//****cfh****Add控制裁剪框最大高度
    public static boolean FREE_CLIPPING = true;//是否自由裁剪  true  自由裁剪  false 非自由裁剪
    ...
    private static float adjustLeft(float x, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (x - imageRect.left < imageSnapRadius)
            ...
            if (x >= Edge.RIGHT.getCoordinate() - MIN_CROP_LENGTH_PX_W)
                resultXHoriz = Edge.RIGHT.getCoordinate() - MIN_CROP_LENGTH_PX_W;

            // Checks if the window is too small vertically
            if (((Edge.RIGHT.getCoordinate() - x) / aspectRatio) <= MIN_CROP_LENGTH_PX_W)
                resultXVert = Edge.RIGHT.getCoordinate() - (MIN_CROP_LENGTH_PX_W * aspectRatio);
    ...
    private static float adjustRight(float x, Rect imageRect, float imageSnapRadius, float aspectRatio) {
        ...
        if (imageRect.right - x < imageSnapRadius)
            ...
            // Checks if the window is too small horizontally
            if (x <= Edge.LEFT.getCoordinate() + MIN_CROP_LENGTH_PX_W)
                resultXHoriz = Edge.LEFT.getCoordinate() + MIN_CROP_LENGTH_PX_W;

            // Checks if the window is too small vertically
            if (((x - Edge.LEFT.getCoordinate()) / aspectRatio) <= MIN_CROP_LENGTH_PX_W) {
                resultXVert = Edge.LEFT.getCoordinate() + (MIN_CROP_LENGTH_PX_W * aspectRatio);
            }
